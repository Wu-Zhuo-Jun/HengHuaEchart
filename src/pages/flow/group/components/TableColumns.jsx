...
export const customerAgeColumns = () => {
  ...
  const columns = [
    ...
    {
        title: Language.SUOSHUJITUAN,
        key: "groupName",
        dataIndex: "groupName",
        align: "center",
        render: (text, record, index) => <div title={record?.groupName || ""}>{ record.groupName? (record.groupName.length > 50 ? record.groupName.substring(0, 50) + "..." : record.groupName) : ""}</div>,
        showSorterTooltip: false,
        onCell: (record, index) => ({ rowSpan: record._rowSpan }),
      },
    {
  ...
  let columns = [
    {
     ...
     {title:Language.SUOSHUJITUAN,key:"groupName",dataIndex:"groupName",align:"center",showSorterTooltip:false},
