...
import { dataViewMap } from "@/data/const";
import Http from "@/config/Http";
import { DashboardPanel } from "./components/panel";
import { TrendChart, RecentSevenDaysChart, Last12MonthsFlowTrendChart, FloorConversionChart, DeviceInfoChart, CustomerPortraitChart } from "./components/charts";
import { FestivalFlowPanel, OutLetFlowPanel, GroupStatisticsPanel } from "./components/panel";
...
const DataView = () => {
  ...
  useEffect(() => {
    // 从sessionStorage读取配置
    const savedConfig = sessionStorage.getItem("dashboardSiteCfg");
    if (savedConfig) {
      try {
        const parsedConfig = JSON.parse(savedConfig);
        // 将数字ID转换为组件value和percentage
        const convertComponents = (components) => {
          if (!components || !Array.isArray(components)) return [];
          return components
            .filter((item) => item != null) // 先过滤掉 null 和 undefined
            .map((item) => {
              // 如果item是数字，说明是旧格式ID，需要转换为组件对象
              if (typeof item === "number") {
                const mapItem = dataViewMap[item];
                return mapItem
                  ? {
                      component: mapItem.value,
                      percentage: mapItem.percentage,
                    }
                  : null;
              }
              // 如果item是对象且有id字段，说明是新格式（保存时包含percentage）
              if (typeof item === "object" && item.id) {
                const mapItem = dataViewMap[item.id];
                return mapItem
                  ? {
                      component: mapItem.value,
                      percentage: item.percentage || mapItem.percentage,
                    }
                  : null;
              }
              // 如果item是对象且有component字段，直接使用
        ...
        const convertedConfig = {
          ...parsedConfig,
          leftComponents: convertComponents(parsedConfig.leftComponents),
          rightComponents: convertComponents(parsedConfig.rightComponents),
        };
        setConfig(convertedConfig);
        siteIdRef.current = parsedConfig.siteId;
        requestDataViewConfig(parsedConfig.siteId);
      } catch (error) {
  ...
  const getComponentLabel = (value) => {
    if (!value) return "";
    const option = Object.values(dataViewMap).find((item) => item.value === value);
  ...
  if (!config) {
    return (
      <div className={isMobile ? "data-view-wrapper" : ""}>
        <div className="data-view-container" style={{ display: "flex", justifyContent: "center", alignItems: "center", color: "#fff" }}>
          加载中...
        </div>
      </div>
    );
    ...
    <div className={isMobile ? "data-view-wrapper" : ""}>
      <div
        ...
        <div className="data-view-header-row">
          <div className="data-view-site-name">{config.showSiteName === 1 ? <div>{config?.siteName || ""}</div> : <div></div>}</div>
          <div className="data-view-title">{config.title || "客流大数据可视化平台"}</div>
          <div className="data-view-time">{formatTime(currentTime)}</div>
        </div>

        {/* 主要内容区域 */}
        <div className="data-view-main-content">
          {/* 左侧列 */}
          <div className="data-view-left-column">
            {config.leftComponents &&
              config.leftComponents
                .filter((item) => item != null && item.component)
                .map((item, index) => {
                  return (
                    <div key={index} className="data-view-component-item" style={{ height: `${item.percentage}%` }}>
          ...
          {/* 中间区域 */}
          <div className="data-view-center-area">
            <div className="data-view-component-item" style={{ flex: 2 }}>
              <div className="component-title">
                <div className="component-title-icon"></div>
            ...
            <div className="data-view-component-item" style={{ flex: 3, display: "flex", flexDirection: "column", gap: "0.2rem" }}>
              ...
              <div style={{ flex: 3, display: "flex", flexDirection: "column" }}>
                <div className="component-title">
                  <div className="component-title-icon"></div>
                  <div className="component-title-text">近12个月趋势图表</div>
                </div>
                <div className="component-content" style={{ flex: 1 }}>
          ...
          {/* 右侧列 */}
          <div className="data-view-right-column">
        ...
        {isNeutralDomain ? null : <div className="data-view-footer-row">由广州恒华科技提供技术支持，经理热线:13380018134梁工</div>}
      </div>
    </div>
  );
};
